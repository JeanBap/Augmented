...
const NON_FINANCE_RE = /\b(software\s*engineer|frontend|backend|fullstack|full-stack|devops|sre\b|data\s*engineer|database\s*developer|t-sql|machine\s*learn|ml\s*engineer|ai\s*engineer|product\s*design|ux\s*design|ui\s*design|graphic\s*design|brand\s*design|content\s*writ|copywriter|marketing\s*(?:manager|specialist|director|lead|coordinator)|paid\s*media|social\s*media|community\s*manager|recruiter|talent\s*acquisition|people\s*ops|hr\s*(?:manager|partner|generalist)|legal\s*counsel|general\s*counsel|sales\s*(?:rep|executive|manager|director|engineer)|(?:senior\s*)?account\s*manager(?!.*account(?:ing|ant))|account\s*executive|customer\s*success|solutions?\s*engineer|security\s*engineer|infrastructure|platform\s*engineer|ios\s*developer|android\s*developer|mobile\s*engineer|qa\s*engineer|test\s*engineer|site\s*reliability|cloud\s*engineer|it\s*consultant|go-to-market|gtm\s*lead|business\s*development\s*(?:rep|manager|praktikum)|bdr\b|sdr\b|customer\s*support|office\s*manager|executive\s*assistant|communications?\s*manager|pr\s*manager|event\s*manager|operations?\s*(?:manager|lead|coordinator)(?!.*financ))/i;
